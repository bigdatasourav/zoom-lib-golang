...
var (
	...
	defaultOAuthClient *OAuthClient
)
...
// NewOauthClient returns a new API client
func NewOAuthClient(accountID string, clientID string, clientSecret string) *OAuthClient {
	var uri = url.URL{
		Scheme: "https",
		Host:   apiURI,
		Path:   apiVersion,
...
func initializeOAuthDefault(c *OAuthClient) *OAuthClient {
	if c == nil {
		if defaultOAuthClient == nil {
			defaultOAuthClient = NewOAuthClient(AccountID, ClientID, ClientSecret)
		}
...
func (c *OAuthClient) executeRequest(opts requestV2Opts) (*http.Response, error) {
	client := c.httpClient()
	req, err := c.addRequestAuth(c.httpRequest(opts))
	if err != nil {
		return nil, err
...
func (c *OAuthClient) httpRequest(opts requestV2Opts) (*http.Request, error) {
	var buf bytes.Buffer
...
func (c *OAuthClient) requestV2(opts requestV2Opts) error {
	// make sure the defaultOAuthClient is not nil if we are using it
	c = initializeOAuthDefault(c)
...
func (c *OAuthClient) requestV2WithBody(opts requestV2Opts, resp *http.Response) error {
	// read HTTP response
	defer resp.Body.Close()
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
...
func (c *OAuthClient) requestV2HeadOnly(resp *http.Response) error {
	if resp.StatusCode != 204 {
		return errors.New(resp.Status)
	}
